...
class Momentum:
    ...
    def update(self, weights, biases, grad_w, grad_b):
        ...
        for i in range(len(weights)):
            self.momentum_w[i] = self.momentum * self.momentum_w[i] + (1 - self.momentum) * grad_w[i]
            self.momentum_b[i] = self.momentum * self.momentum_b[i] + (1 - self.momentum) * grad_b[i]
...
class NAG:
    ...
    def update(self, weights, biases, grad_w, grad_b):
        ...
        for i in range(len(weights)):
            lookahead_w = weights[i] - self.momentum * self.velocity_w[i]
            lookahead_b = biases[i] - self.momentum * self.velocity_b[i]
